...
func step(path_start, path_end):
	if tilemap.map_to_world(path_start) == last_start_tile and tilemap.map_to_world(path_end) == last_end_tile:
		return
	last_start_tile = tilemap.map_to_world(path_start)
	last_end_tile = tilemap.map_to_world(path_end)
	return find_path(path_start, path_end)
...
func add_walkable_exceptions(coordinates, disable_point):
	var tile_coord = world_to_map(coordinates)
	var id = calculate_point_index(tile_coord)
	if disable_point:
